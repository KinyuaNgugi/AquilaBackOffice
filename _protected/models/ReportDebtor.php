...
class ReportDebtor
{
    ...
    public static function buildDebtByClientChart($id,$start_date = null,$end_date = null,$period=null)
    {
        $results = self::getDebtByClient($id,$start_date,$end_date,$period);
        if ($results)
        {
            for ($i=0;$i<count($results);$i++)
            {
    ...
    public static function buildDebtByClientTable($id,$start_date = null,$end_date = null,$period=null)
    {
        $results = self::getDebtByClient($id,$start_date,$end_date,$period);
        if ($results)
        {
    ...
    function getAccountsReceivable($id,$days=null){
    ...
    function getDebtorsByThirtyDayAgeingAnalysis($id){
        $thirty_days=self::getAccountsReceivable($id,30);
        return $thirty_days;
    }
    function getDebtorsBySixtyDayAgeingAnalysis($id){
        $sixty_days_cumulative=self::getAccountsReceivable($id,60);
        $debts_for_last_thirty_days=self::getDebtorsByThirtyDayAgeingAnalysis($id);
        $sixty_days=array();
    ...
    function getDebtorsByNinetyDayAgeingAnalysis($id){
        $ninety_days_cumulative=self::getAccountsReceivable($id,90);
        $debts_for_last_sixty_days=self::getDebtorsBySixtyDayAgeingAnalysis($id);
        $debts_for_last_thirty_days=self::getDebtorsByThirtyDayAgeingAnalysis($id);
        $ninety_days=array();
    ...
    function getDebtorsByOneTwentyDayAgeingAnalysis($id){
        $one_twenty_days_cumulative=self::getAccountsReceivable($id,120);
        $debts_for_last_ninety_days=self::getDebtorsByNinetyDayAgeingAnalysis($id);
        $debts_for_last_sixty_days=self::getDebtorsBySixtyDayAgeingAnalysis($id);
        $debts_for_last_thirty_days=self::getDebtorsByThirtyDayAgeingAnalysis($id);
    ...
    public static function buildDebtorAgeingAnalysisChart($id)
    {
        $thirty_debts = self::getDebtorsByThirtyDayAgeingAnalysis($id);
        $sixty_debts = self::getDebtorsBySixtyDayAgeingAnalysis($id);
        $ninety_debts = self::getDebtorsByNinetyDayAgeingAnalysis($id);
        $one_twenty_debts = self::getDebtorsByOneTwentyDayAgeingAnalysis($id);
    ...
    public static function buildAgeingAnalysisByDebtorTable($id)
    {
        $debtors=self::preprocessDebtors($id);
        ...
        if($debtors != NULL)
        {
            ...
            $html .= "<th>".number_format($total_debt).".00"."</th>";
            $html .= "</tr>";
            $html .= "</tbody></table>";
            return $html;
        }
